...
class BillPayment(AccountsController):
	...
	def validate_bill_status(self):
		...
		if boe.bill_status not in ("已收票-可流通", "提示付款中"):
			frappe.throw(
				_("只能兑付状态为「已收票-可流通」或「提示付款中」的票据，当前状态为：{0}").format(boe.bill_status)
			)
	...
	def validate_payment_amount(self):
		...
		if abs(self.payment_amount - boe.bill_amount) > 0.001:
			frappe.throw(_("兑付金额必须等于票面金额 {0}").format(boe.bill_amount))

	def on_submit(self):
		boe = frappe.get_doc("Bill of Exchange", self.bill_of_exchange)

		# 更新票据状态
		boe.update_status("已结清-已结束")
			...
			self.get_gl_dict(
				{
					...
					"remarks": _("票据兑付 - {0}").format(self.bill_no),
				}
			)
		)
